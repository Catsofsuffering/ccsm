...
- **可选验收reviewer（Optional reviewer）**：例如 opencode 可以提供分析供决策参考，但编排侧保留最终拍板权。opencode 始终是增值项，不是主路径。
- **执行worker（Execution worker）**（Claude Agent Teams）：在边界内实现并返回证据，不做 acceptance 判断，不跑 `spec-review`，不决定 archive。

**流程：** orchestrator -> execution worker -> (optional reviewer) -> acceptance decision -> archive
