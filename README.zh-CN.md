...
## 验收拓扑（Acceptance Topology）

验收拓扑定义了执行返回后谁做什么。它与执行路由是不同的层次。

**拓扑角色：**

- **决策owner（Decision owner）**：Codex 拥有 acceptance 和 archive 的最终决策权，从不委托给执行层。
- **可选中间模型agent层（Optional middle-model layer）**（opencode 或 pi）：启用时可提供预审分析供决策参考，但编排侧保留最终拍板权。opencode 和 pi 是同一类别的reviewer——都是增值项，不是主路径。该层可关闭；关闭后默认workflow继续，不需要任何中间模型reviewer。
- **执行worker（Execution worker）**（Claude Agent Teams）：在边界内实现并返回证据，不做 acceptance 判断，不跑 `spec-review`，不决定 archive。

**流程：** orchestrator -> execution worker -> (optional middle-model layer) -> acceptance decision -> archive

Archive 是显式的高信任操作，由决策owner持有。执行路径和验收路径是分开的关注点。
