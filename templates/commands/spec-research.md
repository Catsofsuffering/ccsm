...
- When guiding the user, always reference `/ccsm:*` commands rather than `/opsx:*`.
...
4. **Launch Configured Exploration Calls**
  ...
  - Use the configured prompt assets under `~/.ccsm/prompts/` instead of any wrapper-specific runtime.
...
7. **Finalize the Proposal Context**
   ...
   - Stop after proposal generation and direct the user to `/ccsm:spec-plan`.

8. **Context Checkpoint**
   - Report current context usage.
   - If context is getting large, suggest clearing before `/ccsm:spec-plan`.
