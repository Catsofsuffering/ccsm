...
- `team-exec` 描述的是 Claude 会话内的执行层行为，不是宿主原生 delegation 的替代入口。
...
5. 回收结果
   - 所有包完成后，先整理完整的 `Execution Return Packet`，如提供 `CCSM_RETURN_PACKET_PATH` 则先落盘，再进入 shutdown / cleanup。
```md
...
6. 结束执行层
   ...
   - 将执行摘要返回给 Codex；若 monitor `outputs` 缺失，Codex 应从 `CCSM_RETURN_PACKET_PATH` 对应的持久化文件回收同一份 packet。
