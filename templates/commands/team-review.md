...
1. 收集材料
   - 读取 `git diff`。
   - 读取最近的 `.ccsm/team-plan/*.md`。
   - 读取 `Execution Return Packet`。
   - 如有 active change，同时读取对应 spec artifacts。
   - 当结构化 monitor 输出可用时，检查其 `sessionStatus`、`runId`、`exitCode`、`outputs` 和 `returnPacketPath` 字段。
   - 若 monitor `outputs` 没有可用的最终 packet，则从 `returnPacketPath` 对应的 CCSM 持久化文件读取 fallback packet，而不是把原始终端文本当成完成证据。
   - Confirm execution workers returned evidence only and did not run `spec-review`, edit active change `tasks.md`, mark tasks complete, archive, or decide acceptance readiness.
...
4. 决策
   ...
   - 若无 Critical：
     ...
     - 指示回到 `/ccsm:spec-impl` 或 `/ccsm:spec-review`
